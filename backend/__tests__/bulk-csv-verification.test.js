...
describe('Bulk CSV Verification Functions Unit Tests', () => {
    ...
    describe('detectEmailColumn Function', () => {
        beforeEach(async () => {
            // Create a CSV file and upload record for detection tests
            ...
            testDb.prepare(`
                ...
                (csv_upload_id, user_id, original_filename, file_path, file_size, has_header, headers, preview_data, row_count, column_count, upload_status, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `).run(
                'test-csv-uuid-789',
                testUserId,
        ...
        test.skip('should detect email column successfully - SKIPPED: Uses Papa.parse streaming', async () => {
            try {
                mockReq.body = { csv_upload_id: 'test-csv-uuid-789' };

                await detectEmailColumn(mockReq, mockRes);
                ...
                const responseData = mockRes.json.mock.calls[0][0].data;
                expect(responseData.confidence).toBeGreaterThan(0);

            } catch (error) {
                console.error('Detect email column test failed:', error);
                throw error;
            }
        });

        test.skip('should calculate confidence correctly - SKIPPED: Uses Papa.parse streaming', async () => {
            try {
                mockReq.body = { csv_upload_id: 'test-csv-uuid-789' };
                ...
                const responseData = mockRes.json.mock.calls[0][0].data;
                expect(responseData.confidence).toBe(100); // 2/2 emails = 100%

            } catch (error) {
                console.error('Confidence calculation test failed:', error);
                throw error;
            }
        });

        test.skip('should handle CSV with no email column - SKIPPED: Uses Papa.parse streaming', async () => {
            try {
                // Create CSV without email column
                const csvContent = 'Name,Company\nJohn,Acme\nJane,Tech';
                const noEmailCsvPath = path.join(uploadDir, 'test-csv-no-email.csv');
                fs.writeFileSync(noEmailCsvPath, csvContent);
                ...
                mockReq.body = { csv_upload_id: 'no-email-csv' };

                await detectEmailColumn(mockReq, mockRes);

                const responseData = mockRes.json.mock.calls[0][0].data;
                expect(responseData.confidence).toBe(0);

                // Clean up
                fs.unlinkSync(noEmailCsvPath);

            } catch (error) {
                console.error('No email column test failed:', error);
                throw error;
            }
        });

        test('should reject detection without CSV upload ID', async () => {
            try {
                mockReq.body = {};
                ...
                console.error('Detection without ID test failed:', error);
                throw error;
            }
        });

        test('should reject detection for non-existent upload', async () => {
            try {
                mockReq.body = { csv_upload_id: 'nonexistent-upload' };
                ...
                console.error('Non-existent upload detection test failed:', error);
                throw error;
            }
        });
    });
